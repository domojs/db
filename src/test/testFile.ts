...
describe('simple query', function ()
{
    it('works with simple commands', async function ()
    {
        ...
        var obj = await store.ModelTest1.where('s1', db.BinaryOperator.Equal, 'pwic').firstOrDefault();
        if (obj)
        {
        ...
        var cnt = await store.ModelTest1.where('s1', db.BinaryOperator.Equal, 'pwic').length();
        assert.notStrictEqual(cnt, 0, 'fount 0 item with s1=pwic');;
        assert.equal(cnt, 1, 'found ' + cnt + ' item with s1=pwic');
        for await (var value of store.ModelTest1.where('s1', db.BinaryOperator.Equal, 'pwic'))
        {
    ...
    it('works with query on non key', async function ()
    {
        ...
        fpe.beginTransaction();
        debugger;
        for await (var obj of store.ModelTest1.where('s2', db.BinaryOperator.Equal, 'pwet'))
            fpe.addCommand(store.ModelTest1.delete(obj));
        await fpe.commitTransaction();
        ...
        try
        {
            var obj = await store.ModelTest1.where('s2', db.BinaryOperator.Equal, 'pwet').singleOrDefault();
        ...
        assert.equal(await store.ModelTest1.where('s2', db.BinaryOperator.Equal, 'pwet').length(), 2, 'we should have 2 objects with s2=pwet')
    });
